...
sub tepl_file_loader_load_async ( *@a ) is export {
  state &loader = do if tepl-version < 5 {

    sub tepl_file_loader_load_async (
      TeplFileLoader $loader,
      gint           $io_priority,
      GCancellable   $cancellable,
                     &progress_callback (goffset, goffset, gpointer), # GFileProgressCallback,
      gpointer       $progress_callback_data,
                     &progress_callback_notify (gpointer),
                     &callback (TeplFileLoader, GAsyncResult, gpointer),
      gpointer       $user_data
    )
      is native(tepl)
    { * }
